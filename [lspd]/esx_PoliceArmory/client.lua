	...
	for k,v in pairs(Config.WeaponsInArmory) do
		...
		for z,x in pairs(weapons) do
			if x == v.weaponHash then
				takenOut = true
				table.insert(elements,{label = v.label .. " --- "..('<span style="color:red;">%s</span>'):format("Poza magazynem"), weaponhash = v.weaponHash, lendable = false})
			end
		end
...
function ListOfAttachments(type,name,weaponhash,attachment,flashlight,scope,suppressor)
	...
	if flashlight then
		...
		if state then
			text = "Latarka: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Latarka: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
		end
	...
	if scope then
		...
		if state then
			text = "Celownik: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Celownik: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
		end
	...
	if suppressor then
		...
		if state then
			text = "Tłumik: "..('<span style="color:green;">%s</span>'):format("Założone")
		else
			text = "Tłumik: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'policeArmory_list_of_attachments', {
		...
		if data.current.value == flashlight then
			if data.current.state then
				newData.label = "Latarka: "..('<span style="color:green;">%s</span>'):format("Założone")
				...
				newData.label = "Latarka: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
		...
		elseif data.current.value == scope then
			if data.current.state then
				newData.label = "Celownik: "..('<span style="color:green;">%s</span>'):format("Założone")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeEquipped,name))
			else
				newData.label = "Celownik: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeRemove,name))
			end
		elseif data.current.value == suppressor then
			if data.current.state then
				newData.label = "Tłumik: "..('<span style="color:green;">%s</span>'):format("Założone")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.SuppressorEquipped,name))
			else
				newData.label = "Tłumik: "..('<span style="color:red;">%s</span>'):format("Ściągnięte")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
