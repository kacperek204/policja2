...
AddEventHandler("esx_policeArmory:weaponTakenOut", function(weapon,giveAmmo)
    ...
    local id = ESX.GetPlayerFromId(source).getIdentifier()
		...
		local message = "**" ..GetPlayerName(source).. "** [" ..xPlayer.getIdentifier().. "] **|**  **Wyciągnął** " .. ESX.GetWeaponLabel(weapon) .. " Ze zbrojowni LSPD **|** " ..DATE
		...
		TriggerClientEvent("esx:showNotification", source, "Wyciągnąłeś ~y~~s~ 1x ~r~" .. ESX.GetWeaponLabel(weapon).."~r~")
    end
...
AddEventHandler("esx_policeArmory:weaponInStock", function(weapon,ammo,giveAmmo)
    ...
    if xPlayer then
		...
		local DATE = os.date("%H:%M (%d.%m.%y)")
		local message = "**" ..GetPlayerName(source).. "** [" ..xPlayer.getIdentifier().. "] **|**  **odłożył** " .. ESX.GetWeaponLabel(weapon) .. " do zbrojowni LSPD **|** " ..DATE
		PerformHttpRequest(""..Config.DiscordWebook.."", function(err, text, headers) end, 'POST', json.encode({username = "Police Armory", content = message}), { ['Content-Type'] = 'application/json' })
        TriggerClientEvent("esx:showNotification", source, "Schowałeś ~y~~s~ 1x ~r~" .. ESX.GetWeaponLabel(weapon) .. "~r~")
    end
